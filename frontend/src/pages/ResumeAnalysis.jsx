...
function ResumeAnalysis() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await api.post('/api/ai/Analyse-Resume', formData, {
  ...
  return (
    <div className="bg-gray-50 min-h-screen w-full px-4 py-8">
      <form onSubmit={onSubmitHandler} className="mx-auto w-full max-w-lg p-6 bg-white rounded-lg border border-gray-200 shadow-sm">
        ...
        <input onChange={(e) => setFile(e.target.files[0])} type='file' accept="application/pdf" className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' required />
